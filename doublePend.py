...
def drawTrail(pendulum):
    ...
    for i in pendulum.lastNPts:
        # Create drop off effect for trail that only starts after trailDropoff value has been reached as a percentage of total length of last n points list
        # Works well but takes a lot of processing time and slows the rendering down a bit
        # To use, uncomment large multiline comment and comment out opacity = 0 below it
        """index+=1
        ...
        # Comment this out if using dropoff trail
        opacity = 0
        ...
        if pendulum.lastNPts.index(i) < len(pendulum.lastNPts)-1:
            ...
            pygame.draw.line(window, (255,opacity,opacity), i, pendulum.lastNPts[pendulum.lastNPts.index(i)+1], 2)      
        else:
            continue
